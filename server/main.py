...
class WeatherService(BaseRequestHandler):
    ...
    def handle(self):
        ...
        #      crc = crc16(REP|<TIMESTAMP>|<VERSION>|<PRODUCT>|<DAYS>|<DAY1>,[DAY2,[DAY3,[DAY4,[DAY(n)...]]]|)
        ...
        if msg and len(msg) > 0:
            ...
            if len(msgbuf) == 4 and msgbuf[1] == "WEATHERBABY" and msgbuf[2] == "1":
                request_days = int(msgbuf[3])
                print("正在查询%s的天气..." % cityname)
                # print(msgbuf[1], msgbuf[2],msgbuf[3])
                # self.request.send("fdsafdsafdsafdsa".encode(conf.default_charset))
                retry = 0
                ...
                if result:
                    ...
                    self.request.send(rsp.encode("utf-8"))
